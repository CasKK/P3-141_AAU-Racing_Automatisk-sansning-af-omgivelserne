...
import math
import glob
from py_helios_smooth_result import CreateDevice, HeliosRunning, HeliosEnd
import threading
kernel = np.ones([5,5], np.uint8)
latest_distance_frame = None
...
def HeliosThread(device, scale_z):
    device ,scale_z, _,_,_,_,_,_,_ = CreateDevice()
    global latest_distance_frame
    with device.start_stream(10):
        while True:
            heatmap, depth = HeliosRunning(device, scale_z)
            latest_distance_frame = (heatmap, depth)
    return
...
def main():
    ...
    while True:
        ...
        if latest_distance_frame is not None:
            heatmap, depth = latest_distance_frame
            cv2.imshow("Helios Heatmap", heatmap)
