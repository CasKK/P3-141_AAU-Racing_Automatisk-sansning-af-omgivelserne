...
def validate_device(device):
    ...
    if 'HLT' in device_model_name_node or 'HTP 'in device_model_name_node:
        global isHelios2
        isHelios2 = True
    ...
    try:
        ...
        print(f"Fejl under ændring af Scan3dCoordinateSelector: {e}")
    # Hvis du har brug for at stoppe eller fortsætte med scriptet, kan du gøre det her.

    scale_z = float(nodemap['Scan3dCoordinateScale'].value)

    operating_mode_initial = nodemap['Scan3dOperatingMode'].value
	
    return scale_z, nodemap, operating_mode_initial
...
def set_nodes(nodemap):
    ...
    nodemap.get_node('Scan3dOperatingMode').value = 'Distance6000mm'


	# set exposure time
    print(f'{TAB1}Setting time selector to Exp1000Us')
    nodemap.get_node('ExposureTimeSelector').value = 'Exp1000Us'
	# set gain
    print(f'{TAB1}Setting conversion gain to low')
    nodemap.get_node('ConversionGain').value = 'Low'
	# set image accumulation
    print(f'{TAB1}Setting accumulation to 4')
    nodemap.get_node('Scan3dImageAccumulation').value = 1
	# Enable spatial filter
    print(f'{TAB1}Enable spatial filter')
    nodemap.get_node('Scan3dSpatialFilterEnable').value = True
	# Enable confidence threshold
    print(f'{TAB1}Enable confidence threshold')
    nodemap.get_node('Scan3dConfidenceThresholdEnable').value = True

    return pixel_format_initial

def Buffer_to_BGR8(buffer_3d, scale_z, device):
    nodemap = device.nodemap
    width = buffer_3d.width
    height = buffer_3d.height
    num_pixels = width * height

    # Convert scale_z to float (avoid 'str' error)
    scale_z = float(scale_z)

    # Cast pdata to correct int16 pointer
    raw_ptr = ctypes.cast(
        buffer_3d.pdata,
        ctypes.POINTER(ctypes.c_int16 * (num_pixels * 4))
    )
    data = np.ctypeslib.as_array(raw_ptr.contents)

    # Reshape to (height, width, 4) → (A,B,C,Y)
    data = data.reshape((height, width, 4))

    C = data[:, :, 2].astype(np.float32)

    # Replace invalid pixels (-32768)
    C[C == -32768] = 0

    # Scale depth
    offset = float(nodemap['Scan3dCoordinateOffset'].value)
    Z = (C + offset) * scale_z
    Z[Z > 6000] = 0


    # Normalize to 0–255 range
    z_clipped = np.clip(Z, 0, 6000)
    Z_norm = ((z_clipped / 6000) *255).astype(np.uint8)

    # Create heatmap (blue→red)
...
def CreateDevice():
    ...
    scale_z, _, operating_mode_initial = validate_device(device)
    ...
    return device ,scale_z, pixelFormat_initial, operating_mode_initial,  exposure_time_initial, conversion_gain_initial, image_accumulation_initial, spatial_filter_initial, confidence_threshold_initial
...
def HeliosEnd(device, pixelFormat_initial, operating_mode_initial,  exposure_time_initial, conversion_gain_initial, image_accumulation_initial, spatial_filter_initial, confidence_threshold_initial):     
        ...
        try:
            ...
            print(f'{TAB1}buffer received')
             ...
             print(f"{TAB1}Error while receiving buffer: {e}")
             return
        ...
        try:
            ...
            print(f'{TAB1}Stream stopped')
             ...
             print(f"{TAB1}Error requering buffer or stopping stream: {e}")
    ...
    # restores initial node values
        try:
            ...
            print(f'{TAB1}Node values restored successfullt')
        except Exception as e:
             print(f"{TAB1}Error restoring node vlaues: {e}")
    ...
    #  is unloading.
        try:
            ...
            print(f'{TAB1}Destroyed all created devices')
             ...
             print(f"{TAB1}Error destroying devices: {e}")
